...
const productController = {
    addProduct: async (req, res) => {
        try {
            ...
            await agenda.now('moderate product images', { productId: newProduct._id });
    ...
    getProductOnSale: async (req, res) => {
        try {
            const products = await Product.find({ isOnSale: true, status: 'available' });
            res.status(200).json(products);
        } catch (error) {
    ...
    getProductByGlobalCategory: async (req, res) => {
        try {
            const products = await Product.find({ globalCategoryId: req.params.globalCategoryId, status: 'available' });
            res.status(200).json(products);
        } catch (error) {
            res.status(500).json({ message: 'Failed to get products by global category!', error });
        }
    },
    ...
    getProductAvailable: async (req, res) => {
        try {
            ...
            res.status(500).json({ message: 'Failed to get available products!', error });
        }
    },
